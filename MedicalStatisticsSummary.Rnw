...
A deterministic model would be $y=f\phi(x,\beta)=\beta_0+\beta_1x$.
Cf.\ a non-deterministic model: $y=f\phi(x,\beta,\epsilon)=\beta_0+\beta_1x+\epsilon$. The general linear model is:
...
\textbf{This gives us the important result: $\hat\beta = (XX^T)^{-1}X^T Y$.}
...
\begin{equation}
\begin{split}
Cov(\hat\beta) = Var(\hat\beta) = Var([(XX^T)^{-1}X^T] Y) =& [(XX^T)^{-1}X^T] \sigma^2 I  [(XX^T)^{-1}X^T]^{T}\\
...
$\alpha_1 + \dots + \alpha_n=\sum_{i=1}^{p} \alpha_i = 0$.
...
Now we have three equations and three parameters, which has a unique solution.
...
where $\bar{x} = \frac{\sum x}{n}$, and $S_{xx}=(\sum x_2) -n\bar{x}^2$. 
