...
When the \textbf{joint} distribution $y_{t_1},\dots,y_{t_k}$ is the same 
...
If $y_t$ has the same distribution as $y_{t+s}$.
...
Note that know $\gamma_h$ for all $h$ tells us what the variance $\sigma^2$ is too, if $\gamma_0=\sigma^2$.
\end{enumerate}
...
As $t\rightarrow \infty$, if $\mid \alpha \mid < 1$ and $y_0$ finite,
...
If $t-i \neq t+h -j$, we have $E[\eps_{t-i}\eps_{t+h-j}]=0$.
...
So, as $t\rightarrow \infty$, $Var(y_t)\rightarrow \infty$, since $\sigma^2>0$. Actually, we don't even need to go so far as to find the limit, we just need to notes that $Var(y_t)$ depends on $t$, so the sequence 
...
The mode is causal, the ACF doesn't depend on t, so 
...
and $y_t = \sum_{j=0}^{q} \beta_j \eps_{t+h-j}$,
we can write the last line above as:

\begin{equation}
\begin{split}
 E[y_t y_{t+h}] = & E[\sum_{i=0}^{q} \sum_{i=0}^{q}
 \beta_i \beta_j \eps_{t-i}\eps_{t+h-j} ]\\
 = & \sum_{i=0}^{q} \sum_{i=0}^{q}
