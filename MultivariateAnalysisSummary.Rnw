...
\begin{equation}
\underbrace{(1 + b_1 B + \dots + b_qB^q){h(B)}_\text{h(B)}\eps_{t}
\end{equation}
...
\begin{equation}
y_{t-h}y_t = 
y_{t-h}(a_1 y_{t-1} + \dots + a_p y_{t-p} + \eps_{t}}) + \underbrac{y_{t-h}(b_1 \eps_{t-1} + \dots + b_q \eps_{t-q})}_\text{Expectation 0}
\end{equation}
