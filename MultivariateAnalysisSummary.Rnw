...
\rho_s =
=\frac{\gamma_{t,s}}{\sqrt{\gamma_{t,s}\gamma_{s,s}}}=\frac{t \sigma^2}{\sqrt{t}\sqrt{s}\sigma^2}= \sqrt{\frac{t}{s}}
\end{equation}
\end{enumerate}
...
\begin{split}
y_t =& x_t^T \beta_t + \eps_t \quad \hbox{observation model}\\
\beta_t =& F\beta_{t-1}+ \zeta_t \quad \hbox{transition model}\\ 
\end{split}
\end{equation}

Where $\eps_t \sim N(0,\sigma^2)$
and
$\zeta_t \sim N(0,Z)$.
E.g., 
