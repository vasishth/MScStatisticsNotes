...
\quad \hat \Delta = (\bar{x}_1 - \bar{x}_2)'S^{-1}(\bar{x}_1 - \bar{x}_2)
 \end{equation}
...
\begin{tabular}{cc}
MA(1) & $r_2, r_3,\dots <$ critical value\\
MA(2) & $r_3, r_4,\dots <$ critical value\\
MA(3) & $r_4, r_5,\dots <$ critical value\\
\end{tabular}
...
\begin{tabular}{cc}
AR(1) & $a_2, a_3,\dots <$ critical value\\
AR(2) & $a_3, a_4,\dots <$ critical value\\
AR(3) & $a_4, a_5,\dots <$ critical value\\
\end{tabular}
...
\textbf{Example from old exam}:

\begin{enumerate}
\item
$a_1 = \rho_1$
\item 
$a_2 = \frac{\rho_1^2}{\rho_1^2 -1}$
...
\begin{split}
x_t =& (1-B^s)^D y_t\\
=& \sum_{j=0}^D {n \choose j}(-1)^j B^{sj} y_t \\
\end{split}
\end{equation}
