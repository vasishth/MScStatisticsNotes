...
expanding the RHS using the binomial theorem: (to-do: define here)
...
\subsection{(S)ARIMA forecast equations}
...
\hline
AR(1)         & $\hat y_{n+i}= a^i y_n$ & $\sigma^2 \frac{1-a^{2i}}{(1-a^2)}$\\
AR(p)         & * & *\\
...
* Translate to SSM and then compute forecasts.
...
\begin{equation}
\begin{split}
P_{t+i\mid t} =& F^i P_{t\mit t} (F^i)^T \\
+& \sum_{k=0}^{i-1} F^k Z (F^k)^T
\end{split}
\end{equation}
