...
The ``power'' on B is telling you the lag, it's not a power.
Maybe it'd be more transparent to write $B(2,y_t)$ instead of $B^2y_t$. But we all know by now that statistics notation sucks.
