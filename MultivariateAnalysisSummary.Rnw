...
\subsection{Some math needed in TS}
...
\subsection{Differencing (this detrends)}
...
to-do add Problem 14 here (also 10 or 11?)
...
$\Phi_s(B^s)= 1-a_1 B^1 -\dots-a_PB^P$

$\phi(B)= 1-a_1 B^1 -\dots-a_p B^p$

$H_s(B^s)= 1+b_1 B^1 +\dots+b_Q B^Q$
...
$r_h \sim N(0,1/n)$, for h=1,2,\dots
...
\subsubsection{One-step predictor}
...
expanding the RHS using the binomial theorem: (to-dp: define here)

\begin{equation}
\begin{split}
x_t  =& y_t+\sum_{j=0}^{d} {n \choose k} (-B) y_t  \\
=&  y_t+ \sum_{j=0}^{d} {n \choose k} (-1)^j y_{t-j}  \\
\end{split}
\end{equation}
...
\subsubsection{Consider the SARIMA case}
...
\begin{split}
x_t =& (1-B^s)^D y_t\\
=& y_t + \sum_{j=0}^D {n \choose j}(-B)^j y_t \\
\end{split}
\end{equation}

Extracting the negation sign and reducing $B^jy_t$,

\begin{equation}
x_t = y_t + \sum_{j=0}^D {n \choose j}(-1)^j y_{y-sj}
\end{equation}

So, for the i-th predicted value,

\begin{equation}
y_{t+i} = x_{t+i} - \sum_{j=0}^D {n \choose j}(-1)^j y_{y-sj}
\end{equation}
...
Estimation is happening sequentially: $y_t$ corresponds to a state $\beta_t$. In filtering we want to estimate $\beta_1$ given data up to t: $t_{1:t}$.
...
\begin{equation}
P(\beta_t \mid y_{1:t}) \approx P(\beta_t\mid y_{1:t-1})P(y_t\mid \beta_t)
\end{equation}

May be easier to see if we single out the datum $y_t$ by writing it separately:

\begin{equation}
P(\beta_t \mid y_{1:t-1},y_t) \approx P(\beta_t\mid y_{1:t-1})P(y_t\mid \beta_t)
\end{equation}
...
$\beta_t \mid y_{t:t} \sim N(\hat \beta_{t\mid t}, P_{t\mid t})$, 
...
\end{equation}
($\beta_{t\mid t}$ is computed using Kalman filter)
...
($P_{t\mid t}$ is computed using Kalman filter)
...
\end{equation}
\item 
If $Z_t =Z$, i.e., if Z is time-invariant, we covariance matrix 
$P_{t+i\mid t}$ takes the simplified form:

\begin{equation}
\begin{split}
P_{t+i\mid t} =& F^i P_{t} (F^i)^T \\
+& \sum_{k=0}^{i-1} F^k Z (F^k)^T
\end{split}
\end{equation}

Compare this with the time-varying version:
